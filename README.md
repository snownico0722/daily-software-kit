...
| 文件 | 干什么用 |
|------|----------|
| `clash全局扩展脚本-v1.2.txt` | 贴进 Clash 客户端的脚本 |
| `clash全局扩展覆写配置示例-v1.2.txt` | 想自己加/删规则时参考 |
...
2. 把 `clash全局扩展脚本-v1.2.txt` 全文粘贴进去并保存，更新订阅
3. 打开「代理」页面，**点进每一个策略组，自己选节点**  
   脚本只会建好这些组，不会替你选好  
   例如：默认代理选常用节点，AI 定向可以单独选美国，未命中流量一般保持直连

**想自己加规则时**，看 `clash全局扩展覆写配置示例-v1.2.txt`，按示例写就行。
